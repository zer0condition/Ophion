...
$output = Join-Path $outDir 'OphionMap.exe'
...
$args = @(
    ...
    (Join-Path $PSScriptRoot 'OphionMap.cpp'),
...
if ($LASTEXITCODE -ne 0) { throw "OphionMap compilation failed with exit code $LASTEXITCODE." }
if (-not (Test-Path $output)) { throw "OphionMap compiler did not create $output." }
